...
class CheckoutSolution:
    ...
    def apply_free_item_offer(self, offer: Offer, item: BasketItem) -> NoReturn:
        if item["sku"] == offer["sku"] and item["quantity"] >= offer["quantity"]:
            ...
            if free_item and not free_item["offer_applied"] and not item["sku"] == offer["free_sku"]:
                ...
                for offer in applicable_offers:
                    ...
                    if offer["offer_type"] == "free_item":
                        ...
                        if offer["free_sku"] == item["sku"] and item["quantity"] % offer["quantity"] == 0:
                            number_of_offers = item["quantity"] // (offer["quantity"] + offer["free_quantity"])
                        for _ in range(number_of_offers):
                            # if item["quantity"] % offer["quantity"] == 0:
                            #     number_of_offers -= offer["free_quantity"]
